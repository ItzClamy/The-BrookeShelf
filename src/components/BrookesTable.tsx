...
// Define the Brookestable component props
interface BrookesTableProps {
  books: Book[];
  setBooks: React.Dispatch<React.SetStateAction<Book[]>>;
  fetchBooks: () => Promise<void>;
}

const BrookesTable: React.FC<BrookesTableProps> = ({
  books,
  setBooks,
  fetchBooks,
}) => {
  ...
  // Set background colors based on the color mode
  const baseBg = useColorModeValue("white", "gray.800");
  const alternateBg = useColorModeValue("gray.100", "gray.700");

  // Set the max height of the table based on the breakpoint
